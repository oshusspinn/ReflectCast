...
const App: React.FC = () => {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            setProgress(5);
            const personaDetails = PERSONAS.filter((p) =>
                selectedPersonas.includes(p.id)
            );
            const scriptData = await generateScript(challenge, personaDetails);
            setProgress(20);

            setLoadingStep("Generating episode artwork...");
            const imageUrl = await generateImage(scriptData);
            setProgress(35);

            setLoadingStep("Giving voice to your panel...");

            const onAudioProgress = (audioProgress: number) => {
                // Audio generation is ~60% of the work (from 35% to 95%)
                const newProgress = 35 + (audioProgress / 100) * 60;
                setProgress(newProgress);
            };

            const { audioUrl, timedScript } = await generateAudio(
                scriptData,
                onAudioProgress
            );

            setProgress(100);
            setPodcast({ ...scriptData, audioUrl, imageUrl, timedScript });
        } catch (e) {
            console.error(e);
            const errorMessage =
                e instanceof Error
    ...
    return (
        <div className="min-h-screen bg-primary-background text-text-primary font-sans flex flex-col items-center p-6 md:p-8">
            ...
            <main className="w-full max-w-2xl flex-grow flex flex-col justify-center items-center">
                {error && (
                    <div
                        className="bg-error/10 border border-error/50 text-error px-4 py-3 rounded-2xl relative mb-6 animate-fade-in"
                        role="alert"
                    >
